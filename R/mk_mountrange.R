...
#'                    the density curves. Possible values are 'x' (default),
#'                    'density' (pdf), 'ndensity' (normalized pdf with max = 1),
#'                    and 'ecdf'.
...
#'                          density curve just touches the baseline of the next
#'                          higher one. Smaller values create a separation between
#'                          the curves, and larger values create more overlap.
#'      \item color_direction: 1 or -1. Sets the order of colors.
#'               If 1 (default), colors are ordered from darkest to lightest.
#'               If -1, ordered from lightest to darkest.
#'      \item hue: A string indicating the colormap option to use. Four options
#'               are available: "magma" (or "A"), "inferno" (or "B"),
#'               "plasma" (or "C", the default option), "viridis" (or "D") and
#'               "cividis" (or "E"). Usually, you want to use "C" (reddish warm/hot)
#'                or 'D' (greenish cool/cold).
