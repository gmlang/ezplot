...
p = plt("budget", "boxoffice", colorby = "made_money")
add_lm_line(p, coef.digits = 8, rr.digits = 3, eq_tb_xpos = "right")

# Don't show fit table when having a colorby because the fit table is the
...
p = plt("rating", "bo_bt_ratio", colorby = "made_money") %>%
